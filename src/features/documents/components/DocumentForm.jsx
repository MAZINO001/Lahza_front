...
import { useCreateDocument, useDocument, useUpdateDocument } from "../hooks/useDocumentsQuery";
...
export function DocumentForm({ document, onSuccess }) {
    ...
    const isEditMode = !!id;
    const { data: existingDocument } = useDocument(isEditMode ? id : null, apiType);
    ...
    useEffect(() => {
        if (existingDocument && isEditMode) {
            ...
            reset({
                ...
                items: (isInvoice ? doc.invoice_services : doc.quote_services)?.map((s) => ({
    ...
    const onSubmit = async (data, status) => {
        ...
        mutation.mutate(
            isEditMode ? { id: document.id, data: payload } : payload,
            {
                onSuccess: () => {
                    onSuccess?.();
                    if (!isEditMode) reset();
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-4 w-full">
            <div className="space-y-4">
                ...
                <div className="flex md:flex-row flex-col justify-end gap-3 mt-8">
                    ...
                    >
                        {isEditMode
                            ? (isInvoice ? "Update & Resend" : "Update & Send")
