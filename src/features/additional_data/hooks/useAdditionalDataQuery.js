    ...
    update: (id, data) => {
        ...
        Object.keys(data).forEach(key => {
            ...
            if (Array.isArray(data[key])) {
                ...
                // Handle file fields - always process them
                if (fileFields.includes(key)) {
                    console.log(`Processing file field ${key} with ${data[key].length} files`);

                    // Always iterate through the array, even if empty
                    // For empty arrays, the loop just doesn't run, but we still "processed" it
                    data[key].forEach((file) => {
                        if (file instanceof File) {
                            console.log(`  Appending File: ${file.name}`);
                            formData.append(`${key}[]`, file);
                        } else if (typeof file === 'object' && file.name && file.size) {
                            // Skip existing files (they already exist on server)
                            console.log(`  Skipping existing file: ${file.name}`);
                        }
                    });

                    // KEY FIX: For empty file arrays, append an empty string marker
                    // This tells the server "this field was processed and is empty"
                    if (data[key].length === 0) {
                        console.log(`  Appending empty marker for ${key}[]`);
        ...
        for (let [key, value] of formData.entries()) {
            if (value instanceof File) {
                ...
                console.log(`  ${key}: ${value === '' ? '[EMPTY MARKER]' : value}`);
            }
        }
