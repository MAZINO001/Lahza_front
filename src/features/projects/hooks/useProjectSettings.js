...
//             queryClient.invalidateQueries({ queryKey: ["projectServices", pid] });
//             queryClient.invalidateQueries({ queryKey: ["transactions", pid] });
//         },
//         onError: (error) => {
...
//             queryClient.invalidateQueries({ queryKey: ["projectTeam", pid] });
//         },
//         onError: (error) => {
...
export function useDeleteProjectService() {
    ...
    return useMutation({
        ...
        onSuccess: (_, { projectId }) => {
            toast.success("Service removed from project!");
            const pid = String(projectId);
            // Invalidate all project-related queries using correct QUERY_KEYS
            queryClient.invalidateQueries({ queryKey: ["projectServices", pid] });
            queryClient.invalidateQueries({ queryKey: ["transactions", pid] });
            queryClient.invalidateQueries({ queryKey: ["project", pid] }); // QUERY_KEYS.project(id)
            queryClient.invalidateQueries({ queryKey: ["projects"] }); // QUERY_KEYS.projects
            queryClient.invalidateQueries({ queryKey: ["projectTeam", pid] });
        },
        onError: (error) => {
            const errorMessage = error?.response?.data?.message || "Failed to remove service from project.";
...
export function useDeleteProjectInvoice() {
    ...
    return useMutation({
        ...
        onSuccess: (_, { projectId }) => {
            ...
            queryClient.invalidateQueries({ queryKey: ["projectInvoices", pid] });
            queryClient.invalidateQueries({ queryKey: ["transactions", pid] });
            queryClient.invalidateQueries({ queryKey: ["invoices", "withoutProjects"] }); // QUERY_KEYS.invoicesWithoutProjects
            queryClient.invalidateQueries({ queryKey: ["project", pid] }); // QUERY_KEYS.project(id)
            queryClient.invalidateQueries({ queryKey: ["projects"] }); // QUERY_KEYS.projects
        },
        onError: (error) => {
