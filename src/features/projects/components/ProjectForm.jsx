...
// //       fetchProjectData();
// //     } else if (isEditMode && project?.id && clients?.length) {
// //       // Handle edit mode
// //       reset({
...
//   // Wait until both project and clients are loaded before rendering the form in edit mode
//   const isEditReady = isEditMode && project?.id && clients?.length > 0;
//   const isCreateReady = !isEditMode && !isCloneModeActive;
//   const isCloneReady =
//     isCloneModeActive &&
//     !isCloneLoading &&
//     currentProject &&
...
//   // Only handle clone mode resets here — edit mode is handled by the loading guard + defaultValues
//   useEffect(() => {
//     if (isCloneReady) {
//       reset({
//         customerName: currentProject?.client_id
//           ? String(currentProject.client_id)
//           : "",
//         name: currentProject?.name || "",
//         description: currentProject?.description || "",
//         invoice_id: currentProject?.invoice_id
//           ? String(currentProject.invoice_id)
//           : "",
//         start_date: currentProject?.start_date || "",
//         estimated_end_date: currentProject?.estimated_end_date || "",
//         status: "pending",
//       });
//     }
//   }, [isCloneReady]);
...
//   // Show loader until data is ready — this causes form to remount with correct defaultValues
//   if (isEditMode && (projectLoading || clientsLoading || !project?.id)) {
//     return (
//       <div className="flex items-center justify-center p-8">
...
//   if (isCloneModeActive && (isCloneLoading || clientsLoading)) {
//     return (
//       <div className="flex items-center justify-center p-8">
//         <div className="text-sm text-muted-foreground">
//           Loading project data for cloning...
      ...
      {selectedClient && (
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {(selectedClient?.client?.company ||
            selectedClient?.client?.ice ||
            selectedClient?.client?.siren) && (
            <Card className="border-border bg-card text-card-foreground shadow-sm sm:col-span-2 lg:col-span-1">
              <CardHeader>
                <CardTitle className="text-lg font-semibold">
                  Company Info
                </CardTitle>
      ...
      <div className="w-full flex flex-col sm:flex-row gap-4">
        ...
        <div className="w-full sm:w-[60%]">
          <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
            ...
            <div
              className={cn(
                "flex items-center space-x-3 rounded-md border bg-card p-2 h-9 w-full sm:w-[24%]",
                "hover:bg-accent/50 transition-colors cursor-pointer select-none",
                "focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2",
              )}
              onClick={() => setDirectProject((prev) => !prev)}
