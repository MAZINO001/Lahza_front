...
import React from "react";
import { Calendar, Users, AlertTriangle, ChevronDown, Clock } from "lucide-react";
import { useEvents } from "../hooks/useCalendarQuery";
...
export default function EventsSummary() {
  ...
  const todaysMeetings = events
    ...
    .sort((a, b) => {
      const dateA = new Date(a.start_date || a.start);
      const dateB = new Date(b.start_date || b.start);
      return dateA - dateB;
    });

  // Calculate upcoming events (next 7 days, excluding today)
  const nextWeek = new Date(today);
  nextWeek.setDate(nextWeek.getDate() + 7);
  ...
  const sections = [
    {
      ...
      items: todaysMeetings.map((event) => {
        const eventDate = new Date(event.start_date || event.start);
        const timeStr = eventDate.toLocaleTimeString("en-US", {
          hour: "numeric",
          minute: "2-digit",
