...
//   useNoInvoiceProject,
//   useUpdateDocument,
// } from "../hooks/useDocuments/useDocumentsQueryData";
...
//   const {data: documents} = useDocuments("invoices");
//   const lastId = Math.max(...documents.map(doc => doc.id));
...
//     if (field === "quantity") {
//       num = Math.max(1, isNaN(num) || num < 1 ? 1 : num);
//     } else if (["rate", "tax_rate", "discount"].includes(field)) {
//       num = Math.max(0, isNaN(num) ? 0 : num);
//     }
//     setValue(`items.${index}.${field}`, num);
//     const quantity = field === "quantity" ? num : Number(watch(`items.${index}.quantity`) || 1);
//     const rate = field === "rate" ? num : Number(watch(`items.${index}.rate`) || 0);
//     const tax = field === "tax_rate" ? num : Number(watch(`items.${index}.tax_rate`) || 0);
//     const discount = field === "discount" ? num : Number(watch(`items.${index}.discount`) || 0);
...
//         <div className="flex md:flex-row flex-col justify-end gap-4 mt-8">
//           <Button
...
import { useForm, Controller, useFieldArray } from "react-hook-form";
import { Button } from "@/components/ui/button";
import { Plus, X, GripVertical, Currency } from "lucide-react";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import { useAuthContext } from "@/hooks/AuthContext";
import { useSubmitProtection } from "@/hooks/spamBlocker";
import FormField from "@/Components/Form/FormField";
...
export function DocumentForm({ type, onSuccess }) {
  ...
  const {data: documents} = useDocuments("invoices");
  const lastId = documents.length > 0 ? Math.max(...documents.map(doc => doc.id)) : 0;
  ...
  useEffect(() => {
    if (document && (isEditMode || isCloneMode || isConvertMode)) {
        ...
        reset({
          ...
          items:
            doc.quote_services?.map((s) => {
              ...
              // Backend already has values in correct currency - use as-is
              return {
                serviceId: Number(s.service_id),
                description: serviceDetails?.description || "",
                quantity: s.quantity,
      ...
      currencyRef.current = doc.currency || "MAD";
    }
  }, [
    document,
    isEditMode,
  ...
  useEffect(() => {
    const subscription = watch((value, { name }) => {
      if (name === "currency") {
        ...
        const oldCurrency = currencyRef.current;
        const items = value.items || [];

        let factor = 1;
        if (oldCurrency === "MAD" && newCurrency !== "MAD") {
          factor = 0.1;
        } else if (oldCurrency !== "MAD" && newCurrency === "MAD") {
          factor = 10;
        }
        ...
        if (factor !== 1) {
          items.forEach((item, index) => {
            ...
            const currentAmount = Number(watch(`items.${index}.amount`) || 0);

            if (currentRate > 0) {
              setValue(
                `items.${index}.rate`,
                Number((currentRate * factor).toFixed(2)),
              );
              setValue(
                `items.${index}.amount`,
                Number((currentAmount * factor).toFixed(2)),
              );
            }
          });
        }
  ...
  const onSubmit = async (data, status) => {
    ...
    const payload = {
      ...
      services: data.items.map((item) => ({
        service_id: Number(item.serviceId),
        quantity: Number(item.quantity),
        rate: Number(item.rate), // Already in correct currency
        tax: Number(item.tax_rate) || 0,
        discount: Number(item.discount || 0),
        individual_total: Number(item.amount), // Already in correct currency
      })),
    };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="p-4 w-full min-h-screen">
      <div className="space-y-4">
        ...
        {isInvoice ? (
          <>
            <div className="flex gap-4 w-full">
              <div className="w-[8%] min-w-[100px]">
                <FormField
                  label="Invoice Id"
                  type="text"
                  className="opacity-80"
                  value={`${formatId(newId)}`}
                  placeholder="Type An Object"
                  disabled={() => true}
                />
              </div>
                              ...
                              render={({ field, fieldState: { error } }) => (
                                <ServiceSelect
                                  ...
                                  onChange={(val) => {
                                    ...
                                    // Service base_price is ALWAYS in MAD - convert if current form currency is not MAD
                                    ...
                                    if (
                                      ...
                                      unitPrice = unitPrice / 10; // Convert MAD to EUR/USD
                                    }
