...
export function AppSidebar(props) {
  ...
  const packs =
    packsData?.packs?.filter(
      (pack) => pack.active_plans && pack.active_plans.length > 0,
    ) ?? [];

  const plansChildren = packsLoading
    ? [{ title: "Loading...", url: `/${role}/plans`, icon: Server }]
    : packs.map((pack) => ({
        title: pack.name,
        url: `/${role}/plans/${pack.id}`,
  ...
  const sidebarData = {
    client: {
      navMain: [
        {
          items: [
            { title: "Dashboard", url: `/${role}/dashboard`, icon: HomeIcon },
            { title: "Calendar", url: `/${role}/calendar`, icon: Calendar1 },
            {
              title: "Invoices",
              url: `/${role}/invoices`,
              icon: IconFileInvoice,
            ...
            },
            {
              ...
              url: `/${role}/plans`,
              icon: Layers,
              children: plansChildren,
            },
            { title: "Payments", url: `/${role}/payments`, icon: DollarSign },
            {
              title: "Receipt",
              url: `/${role}/receipt`,
              icon: ReceiptCentIcon,
